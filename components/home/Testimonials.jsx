        ...
        <div className="grid md:grid-cols-2 gap-6">
          {TESTIMONIALS.map((t) => (
            <div key={t.name} className="cream-card rounded-2xl p-8">
              ...
              <p className="text-lg leading-relaxed mb-6" style={{ color: "#1a2c3c" }}>“{t.quote}”</p>
              <div className="flex items-center gap-4">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={t.photo} alt={t.name} className="w-12 h-12 rounded-full object-cover border-2 border-gold-500/40" />
                <div>
                  <div className="font-semibold" style={{ color: "#0B1C2C" }}>{t.name}</div>
                  <div className="text-sm" style={{ color: "#5a6a7a" }}>{t.role}</div>
                </div>
              </div>
            </div>
          ))}
